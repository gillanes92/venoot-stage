<html>
    ...
    <table>
        ...
        <tr>
            ...
            <td style="font-size: 14px; text-align: center; background-color: #FFFFFF;">{{ $opens->count() }}</td>
        ...
        <tr>
            ...
            <td style="font-size: 14px; text-align: center; background-color: #E7E6E6;">{{ $bounced->count() }}</td>
            <td style="font-size: 14px; text-align: center; background-color: #E7E6E6;"></td>
            <td style="font-size: 14px; text-align: center; background-color: #E7E6E6;"></td>
        </tr>
        ...
        @foreach ($participants as $participant)
            <tr>
<<<<<<< Updated upstream
                <td>{{ isset($participant['name']) ? $participant['name'] : ''}}</td>
                <td>{{ isset($participant['lastname']) ? $participant['lastname'] : '' }}</td>
                <td>{{ $participant['email'] }}</td>

                @foreach($fields as $field)
=======
                <td>{{ is_array($participant->data['name']) ? join(",", $participant->data['name']) : $participant->data['name'] }}</td>
...
>>>>>>> Stashed changes
                    @if(!in_array($field['key'], ['name', 'lastname', 'email']))
                      @if(array_key_exists($field['key'], $participant))
                        <td>{{ $participant[$field['key']] }}</td>
                      @else
                        <td></td>
                      @endif
                    @endif
                ...
                <td></td>
                <td></td>
                <td>{{ $participant['confirmed_at'] ? ($participant['confirmed_status'] ? $participant['confirmed_at'] : "RECHAZO (" . $participant['confirmed_at'] . ")") : ""  }}</td>
