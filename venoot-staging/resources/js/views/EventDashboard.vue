    ...
    <div class="event-dashboard">
            ...
            <v-container fluid fill-height grid-list-md>
                <v-layout wrap>
                    ...
                    <v-flex align-self-start>
                        <span class="primary white--text subtitle pa-1 mr-2">{{ $t('event.title') }} |</span>
                    ...
                    <v-flex xs12 class="d-flex align-center">
                        ...
                        <v-btn dark fab small class="mb-2" color="green" @click="openExportDialog" :loading="exporting">
                            <!-- @click="exportEventExcel" :loading="exporting"> -->
                            <v-icon>mdi-file-excel</v-icon>

                        </v-btn>
                        <v-btn
                            v-if="event.secure_video"
                            dark
                            fab
                            small
                            class="mb-2"
                            color="green"
                            @click="openExportVideoParticipants"
                            :loading="exporting"
                        >
                                        ...
                                        <v-card>
                                            ...
                                            <v-card-text>
                                                <apexchart
                                                    ...
                                                    height="400"
                                                    type="donut"
                                                    :options="optionsReferer"
                                                    :series="seriesReferer"
                                                />
                                            </v-card-text>
                                        </v-card>
                                    </v-col>
                    ...
                    <v-flex xs12 v-if="event.secure_video">
                        <v-expansion-panels>
                            <v-expansion-panel>
                                ...
                                <v-expansion-panel-content class="mt-4">
                                    <v-text-field
                                        v-model="tempSearchSVV"
                                        :placeholder="$t('search').capitalize()"
                                        @focus.prevent
                                        @keydown.enter="searchSVV = tempSearchSVV"
                                        @blur="searchSVV = tempSearchSVV"
                                        solo
                                    ></v-text-field>
                                    <v-data-table
                                        :headers="secureVideoViewsHeaders"
                                        :items="secureVideoViews"
                                        :loading="loading"
                                        class="elevation-1"
                                        :search="searchSVV"
                                    ></v-data-table>
                ...
                <v-card-text>
                    <v-container grid-list-md>
                        <v-layout wrap>
                            <v-flex xs12 sm6 md6>
                                <v-switch
                                    :disabled="event.estimate.landing"
                                    v-model="estimate.landing"
                                    :label="$t('estimate.createLanding')"
                                ></v-switch>
                            </v-flex>
                            <v-flex xs12 sm6 md6>
                                <v-switch
                                    :disabled="event.estimate.confirmation_form"
                                    v-model="estimate.confirmation_form"
                                    :label="$t('estimate.confirmationForm')"
                                ></v-switch>
                            </v-flex>
                            <v-flex xs4>
                                <v-switch
                                    :disabled="event.estimate.mailings"
                                    v-model="estimate.mailings"
                                    :label="$t('estimate.mailings')"
                                ></v-switch>
                            </v-flex>
                            <v-flex xs2>
                                <v-text-field
                            ...
                            <v-flex xs4>
                                <v-switch
                                    :disabled="event.estimate.polls"
                                    v-model="estimate.polls"
                                    :label="$t('estimate.polls')"
                                ></v-switch>
                            </v-flex>
                            <v-flex xs2>
                                <v-text-field
                            ...
                            <v-flex xs4>
                                <v-switch
                                    :disabled="event.estimate.register_keys"
                                    v-model="estimate.register_keys"
                                    :label="$t('estimate.registerKeys')"
                                ></v-switch>
                            </v-flex>
                            <v-flex xs2>
                                <v-text-field
                            ...
                            <v-flex xs12 sm6 md6>
                                <v-switch
                                    :disabled="event.estimate.ticket_sale"
                                    v-model="estimate.ticket_sale"
                                    :label="$t('estimate.ticketSale')"
                                ></v-switch>
                            </v-flex>
                            <v-flex xs12 sm6 md6>
                                <v-switch
                                    :disabled="event.estimate.free_tickets"
                                    v-model="estimate.free_tickets"
                                    :label="$t('estimate.freeTickets')"
                                ></v-switch>
                            </v-flex>
                        </v-layout>
                    </v-container>
        ...
        <v-dialog v-model="landingDialog" persistent scrollable max-width="90%">
            <v-card>
                                ...
                                <v-simple-table>
                                    <template v-slot:default>
                                        <tbody>
                                            ...
                                            <tr
                                                v-if="
                                                    ['full', 'emailing', 'form', 'poll', 'app', 'cart'].includes(
                                                        category
                                                    )
                                                "
                                            >
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="blue" class="white--text" :loading="loading" @click="saveExtraEstimate"
                        >Guardar Cotizacion</v-btn
                    >
                    <v-btn color="black" class="white--text" :loading="loading" @click="landingDialog = false"
                        >Cancelar</v-btn
                    >
                </v-card-actions>
            </v-card>
            ...
            <v-card>
                        ...
                        <v-row>
                            <v-col cols="12">
                                <v-form v-model="isExportFormValid">
                                    <v-text-field
                                        label="Correo Electronico para Envio"
                                        v-model="exportMail"
                                        :rules="[
                                            (v) =>
                                                !v ||
                ...
                <v-card-actions>
                    ...
                    <v-btn
                        ...
                        :loading="loading"
                        @click="exportEventExcel"
                        >enviar</v-btn
                    >
                    <v-btn color="grey" class="white--text" :loading="loading" @click="exportDialog = false"
                        >cancelar</v-btn
                    >
                </v-card-actions>
            </v-card>
